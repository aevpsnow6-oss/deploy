...
- ANTES de escribirlo, COMPRUEBA el alcance: `start_line` dice cuántos criterios se van a evaluar. Si no coincide con el alcance que acabas de confirmar con el usuario (p. ej. acordasteis 35 criterios y dice «rúbrica completa (76 criterios)»), NO continúes: dilo claramente, explica que el filtro no se aplicó y pide al usuario que avise al administrador antes de repetir. El trabajo ya lanzado consume presupuesto.
- Si `start_line` incluye un AVISO de parámetros no reconocidos, el servidor está desactualizado: repórtalo tal cual y no sigas.
